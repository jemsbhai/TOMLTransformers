...
\subsection{Attention Kernel}

At matched shape and precision, standard attention costs 1.9x the energy of
the fused kernel at $s = 512$, rising to 12.0x at $s = 4096$ (eight pairs,
instrument~B, pre-registered as a report-only derived quantity). The excess
energy scales exactly as the score-matrix accounting of
Section~\ref{sec:model} predicts: quadratically in $s$, with each doubling
multiplying it by 4.0 to 4.9, and linearly in depth, with GPT-2's excess
2.0x DistilGPT2's at every $s$, matching its layer count. The same pairs on
the A100 give 3.7x to 6.9x with the same two scalings. The two kernels
perform identical arithmetic; FLOP counts assign them identical cost while
measured energy differs by up to 12x.
...
pre-registration. Table~\ref{tab:baselines} also reports each roofline
fit's average power against the part's rated envelope.
